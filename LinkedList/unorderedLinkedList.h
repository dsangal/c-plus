...
void unorderedLinkedList<Type>::deleteAll(const Type& deleteItem)
{
    ...
    {
        if (current->info == deleteItem)     //deleteItem is found
...
{
    int smallestNbr = 999999;                //variable to store smallest nbr
    ...
    while (current != NULL)                 //search the list for smallest nbr
    {
        if (current->info < smallestNbr)     //compare current item with
            //the current smallest nbr
            
            smallestNbr = current->info;     //save current smallest item
        
        current = current->link;             //make current point to the next node
    }
    
    deleteNode(smallestNbr);                 //delete smallest item afer finding
    //it in the while code above
}
